...
Supports two modes controlled by config:
  bdd: false  → classic spec + Page Object Model (native Playwright)
  bdd: true   → Cucumber .feature file + step definitions + self-healing POM
                (requires playwright-bdd)
...
class PlaywrightImplementerAgent(BaseAgent):
    """Renders Playwright test files (specs or Cucumber) using Jinja2 templates."""
    ...
    # ------------------------------------------------------------------
    # Mode: BDD (Cucumber / playwright-bdd)
    # ------------------------------------------------------------------
    ...
    def _generate_bdd(
        ...
        generated.append(self._render(
            template="playwright_feature.j2",
            dest=self.output_dir / "features" / f"{ticket_id}.feature",
            context=ctx,
        ))

        # Step definitions
        generated.append(self._render(
            template="playwright_steps.j2",
            dest=self.output_dir / "steps" / f"{ticket_id}.steps.{self.ext}",
            context=ctx,
        ))
